...
static NSString *kIASKCredits = @"Powered by Johnny Xu"; // Leave this as-is!!!
...
- (void)viewWillAppear:(BOOL)animated
{
	...
	if (_showDoneButton)
    {
        // Change by Johnny Xu, 2015.6.16
        NSString *fontName = GBLocalizedString(@"FontName");
        CGFloat fontSize = 18;
//        UIBarButtonItem *rightItem = [[UIBarButtonItem alloc] initWithTitle:GBLocalizedString(@"Done") style:UIBarButtonItemStylePlain target:self action:@selector(dismiss:)];
...
//        self.navigationItem.rightBarButtonItem = rightItem;
        ...
        UIBarButtonItem *leftItem = [[UIBarButtonItem alloc] initWithTitle:GBLocalizedString(@"Back") style:UIBarButtonItemStylePlain target:self action:@selector(dismiss:)];
...
//		self.navigationItem.rightBarButtonItem = buttonItem;
	...
	if (!self.title)
    {
		self.title = GBLocalizedString(@"Settings");
	}
...
- (void)dismissWithNotSave:(id)sender
{
